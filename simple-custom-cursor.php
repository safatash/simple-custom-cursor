...
───────────────────────────────────────────── */
register_uninstall_hook( __FILE__, 'scc_uninstall' );
function scc_uninstall() {
    delete_option( 'scc_options' );
}

/* ─────────────────────────────────────────────
   Defaults & options helper
───────────────────────────────────────────── */
function scc_defaults() {
    return array(
...
function scc_get_options() {
    return wp_parse_args( get_option( 'scc_options', array() ), scc_defaults() );
}
...
function scc_register_settings() {
    register_setting( 'scc_options_group', 'scc_options', array( 'sanitize_callback' => 'scc_sanitize_options' ) );
}
...
/* ─────────────────────────────────────────────
   Admin assets
───────────────────────────────────────────── */
add_action( 'admin_enqueue_scripts', 'scc_admin_assets' );
function scc_admin_assets( $hook ) {
    if ( 'settings_page_simple-custom-cursor' !== $hook ) return;
    wp_enqueue_style(  'scc-admin-style',  SCC_PLUGIN_URL . 'admin/admin.css', array(), SCC_VERSION );
    wp_enqueue_script( 'scc-admin-script', SCC_PLUGIN_URL . 'admin/admin.js',  array(), SCC_VERSION, true );
}

/* ─────────────────────────────────────────────
   Frontend assets
───────────────────────────────────────────── */
add_action( 'wp_enqueue_scripts', 'scc_enqueue_frontend' );
function scc_enqueue_frontend() {
    $o = scc_get_options();

    wp_enqueue_style( 'scc-style', SCC_PLUGIN_URL . 'assets/css/cursor.css', array(), SCC_VERSION );

    $css = sprintf(
        ':root{--scc-dot-size:%dpx;--scc-outline-size:%dpx;--scc-outline-wt:%dpx;--scc-dot-color:%s;--scc-outline-color:%s;--scc-blend:%s;}',
        absint( $o['dot_size'] ),
    ...
    wp_add_inline_style( 'scc-style', $css );

    wp_enqueue_script( 'scc-script', SCC_PLUGIN_URL . 'assets/js/cursor.js', array(), SCC_VERSION, true );
    wp_localize_script( 'scc-script', 'sccSettings', array(
...
function scc_settings_page() {
    ...
    <div class="wrap scc-wrap">
        ...
        <div class="scc-header">
            <div class="scc-header-icon">🖱️</div>
        ...
        <form method="post" action="options.php">
            ...
            <div class="scc-layout">
                ...
                <!-- ── Sidebar navigation ── -->
                <nav class="scc-sidebar" aria-label="<?php esc_attr_e( 'Settings sections', 'simple-custom-cursor' ); ?>">
                    <ul class="scc-nav">
                        <li>
                            <button type="button" class="scc-nav-btn active" data-tab="style">
                                <span class="scc-nav-icon">🎨</span>
                                <?php esc_html_e( 'Style', 'simple-custom-cursor' ); ?>
                            </button>
                        </li>
                        <li>
                            <button type="button" class="scc-nav-btn" data-tab="colors">
                                <span class="scc-nav-icon">🌈</span>
                                <?php esc_html_e( 'Colors', 'simple-custom-cursor' ); ?>
                            </button>
                        </li>
                        <li>
                            <button type="button" class="scc-nav-btn" data-tab="size">
                                <span class="scc-nav-icon">📐</span>
                                <?php esc_html_e( 'Size', 'simple-custom-cursor' ); ?>
                            </button>
                        </li>
                        <li>
                            <button type="button" class="scc-nav-btn" data-tab="behaviour">
                                <span class="scc-nav-icon">⚡</span>
                ...
                <div class="scc-content">
                    ...
                    <div class="scc-preview-panel">
                        <div class="scc-preview-header">
                            <h3><?php esc_html_e( 'Live Preview', 'simple-custom-cursor' ); ?></h3>
                            <span class="scc-preview-badge"><?php esc_html_e( 'Interactive', 'simple-custom-cursor' ); ?></span>
                        </div>
                        <div class="scc-preview-stage" id="scc-preview-stage">
                            <span class="scc-preview-center-text"><?php esc_html_e( 'Hover here', 'simple-custom-cursor' ); ?></span>
                            <div id="scc-prev-dot"></div>
                            <div id="scc-prev-outline"></div>
                            <div id="scc-prev-ch"></div>
                            <div id="scc-prev-cv"></div>
                        </div>
                    </div>
                    ...
                    <!-- ════════════════════════════
                         TAB: Style
                    ════════════════════════════ -->
                    <div class="scc-tab active" id="scc-tab-style">
                        <div class="scc-panel">
                            <p class="scc-panel-title">
                                <span class="scc-title-icon">🎨</span>
                                <?php esc_html_e( 'Cursor Style', 'simple-custom-cursor' ); ?>
                            </p>
                            <p class="scc-panel-desc"><?php esc_html_e( 'Choose the visual style of the cursor shown on your site.', 'simple-custom-cursor' ); ?></p>

                            <div class="scc-type-grid">
                                <?php foreach ( $types as $value => $meta ) : ?>
                                <label class="scc-type-card <?php echo ( $o['cursor_type'] === $value ) ? 'active' : ''; ?>">
                                    <input type="radio" name="scc_options[cursor_type]" value="<?php echo esc_attr( $value ); ?>" <?php checked( $o['cursor_type'], $value ); ?>>
                                    <div class="scc-type-preview">
                                        <?php echo wp_kses( scc_get_type_svg( $meta['svg_key'] ), scc_svg_kses_args() ); ?>
                                    </div>
                                    <span class="scc-type-label"><?php echo esc_html( $meta['label'] ); ?></span>
                                </label>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    ...
                    <!-- ════════════════════════════
                         TAB: Colors
                    ════════════════════════════ -->
                    <div class="scc-tab" id="scc-tab-colors">
                        <div class="scc-panel">
                            <p class="scc-panel-title">
                                <span class="scc-title-icon">🌈</span>
                                <?php esc_html_e( 'Cursor Colors', 'simple-custom-cursor' ); ?>
                            </p>
                            <p class="scc-panel-desc"><?php esc_html_e( 'Set individual colors for the dot and the outline ring.', 'simple-custom-cursor' ); ?></p>

                            <!-- Dot color -->
                            <div class="scc-field-row">
                                <div class="scc-field-label">
                                    <?php esc_html_e( 'Dot Color', 'simple-custom-cursor' ); ?>
                                    <span class="scc-field-sub"><?php esc_html_e( 'The small inner dot', 'simple-custom-cursor' ); ?></span>
                                </div>
                                <div class="scc-field-control">
                                    <div class="scc-color-swatch">
                                        <div class="scc-color-dot" style="background:<?php echo esc_attr( $o['dot_color'] ); ?>"></div>
                                        <input type="color" id="scc_dot_color_picker" data-sync="scc_dot_color_hex" value="<?php echo esc_attr( $o['dot_color'] ); ?>">
                                    </div>
                                    <input type="text" id="scc_dot_color_hex" name="scc_options[dot_color]" value="<?php echo esc_attr( $o['dot_color'] ); ?>" class="scc-hex-input" maxlength="7" placeholder="#ffffff" aria-label="<?php esc_attr_e( 'Dot color hex', 'simple-custom-cursor' ); ?>">
                                </div>
                            </div>
                            ...
                            <div class="scc-field-row">
                                <div class="scc-field-label">
                                    ...
                                    <span class="scc-field-sub"><?php esc_html_e( 'The trailing ring or circle', 'simple-custom-cursor' ); ?></span>
                                </div>
                                <div class="scc-field-control">
                                    <div class="scc-color-swatch">
                                        <div class="scc-color-dot" style="background:<?php echo esc_attr( $o['outline_color'] ); ?>"></div>
                                        <input type="color" id="scc_outline_color_picker" data-sync="scc_outline_color_hex" value="<?php echo esc_attr( $o['outline_color'] ); ?>">
                                    </div>
                                    <input type="text" id="scc_outline_color_hex" name="scc_options[outline_color]" value="<?php echo esc_attr( $o['outline_color'] ); ?>" class="scc-hex-input" maxlength="7" placeholder="#ffffff" aria-label="<?php esc_attr_e( 'Outline color hex', 'simple-custom-cursor' ); ?>">
                                </div>
                            </div>

                            <hr class="scc-divider">

                            <!-- Blend mode -->
                            <div class="scc-field-row">
                                <div class="scc-field-label">
                                    <?php esc_html_e( 'Blend Mode', 'simple-custom-cursor' ); ?>
                                    <span class="scc-field-sub"><?php esc_html_e( 'How the cursor blends with page content', 'simple-custom-cursor' ); ?></span>
                                </div>
                                <div class="scc-field-control">
                                    <select name="scc_options[blend_mode]" class="scc-select">
                    ...
                    <!-- ════════════════════════════
                         TAB: Size
                    ════════════════════════════ -->
                    <div class="scc-tab" id="scc-tab-size">
                        <div class="scc-panel">
                            <p class="scc-panel-title">
                                <span class="scc-title-icon">📐</span>
                                <?php esc_html_e( 'Size & Weight', 'simple-custom-cursor' ); ?>
                            </p>
                            <p class="scc-panel-desc"><?php esc_html_e( 'Control the dimensions of each cursor element.', 'simple-custom-cursor' ); ?></p>
                            ...
                            foreach ( $sliders as $sl ) :
                                $badge_id = 'scc_badge_' . $sl['name'];
                                $min = $sl['min']; $max = $sl['max'];
                                $pct = round( ( ( $sl['value'] - $min ) / ( $max - $min ) ) * 100 );
                            ?>
                            <div class="scc-slider-row">
                                <div class="scc-slider-header">
                                    <span class="scc-slider-label"><?php echo esc_html( $sl['label'] ); ?></span>
                                    <span class="scc-slider-value-badge" id="<?php echo esc_attr( $badge_id ); ?>"><?php echo esc_html( $sl['value'] . $sl['suffix'] ); ?></span>
                                </div>
                                <p class="scc-slider-hint"><?php echo esc_html( $sl['hint'] ); ?></p>
                                <input
                                    type="range"
                                    class="scc-range-visible"
                                    name="scc_options[<?php echo esc_attr( $sl['name'] ); ?>]"
                                    ...
                                    style="--scc-fill:<?php echo esc_attr( $pct ); ?>%"
                                >
                            </div>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <!-- ════════════════════════════
                         TAB: Behaviour
                    ════════════════════════════ -->
                    <div class="scc-tab" id="scc-tab-behaviour">
                        <div class="scc-panel">
                            <p class="scc-panel-title">
                                <span class="scc-title-icon">⚡</span>
                                <?php esc_html_e( 'Behaviour', 'simple-custom-cursor' ); ?>
                            </p>
                            <p class="scc-panel-desc"><?php esc_html_e( 'Fine-tune animation and interaction settings.', 'simple-custom-cursor' ); ?></p>
                            ...
                            <!-- Mobile toggle -->
                            <div class="scc-toggle-row">
                                <div class="scc-toggle-info">
                                    <span class="scc-toggle-title"><?php esc_html_e( 'Enable on Mobile / Touch', 'simple-custom-cursor' ); ?></span>
                                    <span class="scc-toggle-desc"><?php esc_html_e( 'Off by default. Touch devices use the native cursor unless enabled.', 'simple-custom-cursor' ); ?></span>
                                </div>
                                <label class="scc-toggle" aria-label="<?php esc_attr_e( 'Enable on mobile', 'simple-custom-cursor' ); ?>">
                                    <input type="checkbox" name="scc_options[enable_on_mobile]" value="1" <?php checked( $o['enable_on_mobile'], 1 ); ?>>
                    ...
                    <!-- ── Save bar ── -->
                    <div class="scc-save-bar">
                        <span class="scc-save-note"><?php esc_html_e( 'Changes apply to the frontend immediately after saving.', 'simple-custom-cursor' ); ?></span>
                        <button type="submit" class="scc-save-btn">
